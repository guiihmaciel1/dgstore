<x-app-layout>
    <x-slot name="title">Mensagens WhatsApp</x-slot>
    <div class="py-6">
        <div class="px-6 lg:px-8" x-data="whatsappMessages()">

            <!-- Header -->
            <div style="margin-bottom: 1.25rem;">
                <h1 style="font-size: 1.5rem; font-weight: 700; color: #e3e3e3;">Mensagens WhatsApp</h1>
                <p style="font-size: 0.875rem; color: #818181;">Templates prontos para copiar ou enviar direto</p>
            </div>

            <!-- Telefone do cliente -->
            <div style="margin-bottom: 1rem; background: #141414; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.75rem; padding: 0.875rem 1.25rem;">
                <div style="display: flex; align-items: center; gap: 0.75rem;">
                    ...
                    <input type="text" x-model="phone" placeholder="Telefone do cliente (ex: 5517996498338)"
                           style="flex: 1; padding: 0.375rem 0.625rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; font-size: 0.875rem; outline: none;"
                           onfocus="this.style.borderColor='#16a34a'" onblur="this.style.borderColor='rgba(255,255,255,0.06)'">
                </div>
            </div>
                ...
                <template x-for="cat in categories" :key="cat.key">
                    <button @click="filterCat = filterCat === cat.key ? '' : cat.key" type="button"
                            :style="filterCat === cat.key
                                ...
                                : 'padding: 0.375rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 500; border: 1px solid rgba(255,255,255,0.06); cursor: pointer; background: #141414; color: #818181;'"
                            x-text="cat.label">
                    </button>
                </template>
            </div>
                ...
                <template x-for="(tpl, idx) in filteredTemplates" :key="tpl.key">
                    <div style="background: #141414; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.75rem; overflow: hidden;">
                        ...
                        <button @click="toggle(tpl.key)" type="button"
                                style="width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 0.875rem 1.25rem; background: #141414; border: none; cursor: pointer; text-align: left;">
                            <div style="display: flex; align-items: center; gap: 0.625rem;">
                                <span style="font-size: 1.125rem;" x-text="tpl.icon"></span>
                                <div>
                                    <span style="font-size: 0.9375rem; font-weight: 600; color: #e3e3e3;" x-text="tpl.title"></span>
                                    <div style="font-size: 0.6875rem; color: #666666;" x-text="tpl.subtitle"></div>
                                </div>
                            </div>
                            <svg width="16" height="16" :style="openKey === tpl.key ? 'transform:rotate(180deg);' : ''" style="flex-shrink:0; color: #666666; transition: transform 0.2s;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-show="openKey === tpl.key" x-transition style="border-top: 1px solid #f3f4f6; padding: 1rem 1.25rem;">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 0.5rem; margin-bottom: 0.75rem;">
                                <template x-for="(field, fIdx) in tpl.fields" :key="fIdx">
                                    <div style="flex: 1; min-width: 140px;">
                                        <label style="font-size: 0.7rem; font-weight: 500; color: #818181; display: block; margin-bottom: 2px;" x-text="field.label"></label>
                                        <input type="text" x-model="field.value"
                                               style="width: 100%; padding: 0.375rem 0.5rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; font-size: 0.8rem; outline: none;"
                            ...
                            <div style="background: #dcf8c6; border-radius: 0 12px 12px 12px; padding: 0.75rem 1rem; margin-bottom: 0.75rem; font-size: 0.85rem; color: #e3e3e3; white-space: pre-wrap; line-height: 1.5; max-height: 300px; overflow-y: auto;"
                            ...
                            <div style="display: flex; gap: 0.5rem;">
                                ...
                                <a :href="phone.replace(/\D/g, '') ? waLink(tpl) : '#'" target="_blank"
                                   :style="phone.replace(/\D/g, '') ? 'padding: 0.5rem 1rem; border-radius: 0.5rem; font-size: 0.8rem; font-weight: 600; background: #16a34a; color: white; text-decoration: none; display: flex; align-items: center; gap: 6px;' : 'padding: 0.5rem 1rem; border-radius: 0.5rem; font-size: 0.8rem; font-weight: 600; background: #d1d5db; color: #666666; text-decoration: none; display: flex; align-items: center; gap: 6px; pointer-events: none; cursor: not-allowed;'"
