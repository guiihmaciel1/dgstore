...
<div style="background: #141414; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.75rem; overflow: hidden;">
    <button @click="toggleModule('batalha')" type="button"
        ...
        <div style="display: flex; align-items: center; gap: 0.75rem;">
            ...
            <div>
                <span style="font-size: 1rem; font-weight: 700; color: #e3e3e3;">Batalha de Specs — iPhone vs iPhone</span>
                <div style="font-size: 0.75rem; color: #666666;">Comparativos que aparecem toda hora na loja</div>
            </div>
        </div>
        <div style="display: flex; align-items: center; gap: 0.5rem;">
            <span x-show="readModules.includes('batalha')" style="font-size: 0.6875rem; font-weight: 600; color: #059669; background: rgba(16,185,129,0.1); padding: 2px 8px; border-radius: 9999px;">✓ lido</span>
            <svg width="16" height="16" :style="openModule === 'batalha' ? 'transform:rotate(180deg);' : ''" style="flex-shrink:0; color: #666666; transition: transform 0.2s;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
            <template x-for="battle in battles" :key="battle.title">
                <div style="border: 1px solid rgba(255,255,255,0.06); border-radius: 0.75rem; overflow: hidden;">
                    <div style="background: #1a1a1a; padding: 0.75rem 1rem; display: flex; align-items: center; justify-content: space-between;">
                        <span style="font-size: 0.875rem; font-weight: 700; color: #e3e3e3;" x-text="'⚡ ' + battle.title"></span>
                    </div>
                    <div style="padding: 1rem;">
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; margin-bottom: 0.75rem;">
                            <template x-for="(side, idx) in battle.sides" :key="idx">
                                <div style="text-align: center;">
                                    <div style="font-size: 0.8125rem; font-weight: 700; color: #e3e3e3; margin-bottom: 0.375rem;" x-text="side.name"></div>
                                    <template x-for="spec in side.highlights" :key="spec">
                                        <div style="font-size: 0.75rem; color: #818181; padding: 2px 0;" x-text="spec"></div>
                                    </template>
                                </div>
                            </template>
                        </div>
                        <div style="background: rgba(16,185,129,0.1); border-radius: 0.5rem; padding: 0.625rem 0.75rem;">
                            <p style="font-size: 0.8rem; color: #6ee7b7; font-weight: 500;" x-text="'🗣️ ' + battle.script"></p>
                        </div>
                    </div>
                </div>
            </template>
