            ...
            @if($selectedSupplier)
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                    ...
                    <div style="background: white; border: 1px solid #e5e7eb; border-radius: 0.75rem; overflow: hidden;">
                        ...
                        <div style="padding: 1rem; font-family: monospace; font-size: 0.8rem; white-space: pre-wrap; background: #f9fafb; max-height: 500px; overflow-y: auto; line-height: 1.6; color: #374151;">{{ $availableText = "📦 *ESTOQUE CONSIGNADO - DISPONÍVEL*\n━━━━━━━━━━━━━━━━━━━\nFornecedor: {$selectedSupplier->name}\nData: " . now()->format('d/m/Y') . "\n" }}@if($available->isEmpty()){{ "\nNenhum item disponível." }}@else @php $totalAvail = 0; @endphp @foreach($available as $ai){{ "\n▸ {$ai->full_name}" }}@if($ai->imei){{ "\n  IMEI: {$ai->imei}" }}@endif{{ " | Custo: R$ " . number_format($ai->supplier_cost, 2, ',', '.') }}@php $totalAvail += $ai->supplier_cost * $ai->available_quantity; @endphp @endforeach{{ "\n\nTotal: {$available->count()} aparelho(s) | R$ " . number_format($totalAvail, 2, ',', '.') }}@endif</div>
                    </div>
                    ...
                    <div style="background: white; border: 1px solid #e5e7eb; border-radius: 0.75rem; overflow: hidden;">
                        ...
                        <div style="padding: 1rem; font-family: monospace; font-size: 0.8rem; white-space: pre-wrap; background: #f9fafb; max-height: 500px; overflow-y: auto; line-height: 1.6; color: #374151;">{{ $soldText = "📊 *ESTOQUE CONSIGNADO - VENDIDOS*\n━━━━━━━━━━━━━━━━━━━\nFornecedor: {$selectedSupplier->name}\nPeríodo: " . \Carbon\Carbon::parse($dateFrom)->format('d/m') . " a " . \Carbon\Carbon::parse($dateTo)->format('d/m/Y') . "\n" }}@if($sold->isEmpty()){{ "\nNenhuma venda no período." }}@else @php $totalSold = 0; @endphp @foreach($sold as $si){{ "\n▸ {$si->full_name}" }}@if($si->imei){{ "\n  IMEI: {$si->imei}" }}@endif{{ " | Custo: R$ " . number_format($si->supplier_cost, 2, ',', '.') }}{{ "\n  Vendido em: " . ($si->sold_at ? $si->sold_at->format('d/m/Y') : '-') }}@php $totalSold += $si->supplier_cost * $si->quantity; @endphp @endforeach{{ "\n\nTotal: {$sold->count()} aparelho(s) | R$ " . number_format($totalSold, 2, ',', '.') }}@endif</div>
                    </div>
                </div>
            @else
