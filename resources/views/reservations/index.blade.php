                                    ...
                                    <tr style="border-bottom: 1px solid #f3f4f6;">
                                        ...
                                        <td style="padding: 0.75rem 1rem;">
                                            ...
                                            @if($reservation->status->isActive())
                                                @php
                                                    $prodKey = $reservation->product_description ?? $reservation->product?->full_name ?? $reservation->product?->name;
                                                    $bestQ = $prodKey ? ($bestQuotationsMap[$prodKey] ?? null) : null;
                                                @endphp
                                                @if($bestQ)
                                                    <div style="margin-top: 0.25rem; font-size: 0.675rem; color: #2563eb; display: flex; align-items: center; gap: 0.25rem;" title="Melhor cotação: {{ $bestQ->supplier->name }} — R$ {{ number_format($bestQ->total_cost, 2, ',', '.') }} com frete">
                                                        <svg style="width: 0.7rem; height: 0.7rem; flex-shrink: 0;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/>
                                                        </svg>
                                                        <span>{{ $bestQ->supplier->name }}: <strong>R$ {{ number_format($bestQ->total_cost, 2, ',', '.') }}</strong></span>
                                                    </div>
                                                @endif
                                            @endif
                                        </td>
