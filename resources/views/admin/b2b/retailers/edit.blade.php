<x-b2b-admin-layout>
    <x-slot name="header">
        <div class="flex min-w-0 items-center gap-3">
            <a href="{{ route('admin.b2b.retailers.show', $retailer) }}" class="-ml-2 shrink-0 rounded-xl p-2 text-gray-400 transition-all duration-200 hover:bg-gray-100 hover:text-gray-600" aria-label="Voltar">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
                </svg>
            </a>
            <div class="min-w-0">
                <h2 class="truncate text-xl font-semibold tracking-tight text-gray-900 sm:text-2xl">Editar: {{ $retailer->store_name }}</h2>
                <p class="mt-0.5 text-sm text-gray-500">Atualize os dados do lojista</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-3xl">
        <form method="POST" action="{{ route('admin.b2b.retailers.update', $retailer) }}" class="space-y-5 sm:space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="apple-card p-5 transition-all duration-200 sm:p-6">
                <div class="mb-4 flex items-center gap-3 sm:mb-5">
                    ...
                    <h3 class="apple-section-title !mb-0">Alterar senha</h3>
                </div>

                <div>
                    <label for="password" class="apple-label">Nova Senha</label>
                    <input id="password" type="text" name="password" value=""
                           class="apple-input rounded-xl shadow-sm transition-all duration-200"
                           placeholder="Deixe vazio para manter a atual"/>
                    <p class="mt-1.5 text-xs text-gray-500">Mínimo 6 caracteres. Deixe vazio para não alterar.</p>
                    @error('password') <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="flex flex-col-reverse gap-3 pt-1 sm:flex-row sm:items-center">
                <a href="{{ route('admin.b2b.retailers.show', $retailer) }}" class="apple-btn-secondary justify-center rounded-xl shadow-sm transition-all duration-200 sm:justify-start">
                    Cancelar
                </a>
