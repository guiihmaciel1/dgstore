<x-perfumes-admin-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-gray-800">Editar Lojista</h2>
    </x-slot>
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <form method="POST" action="{{ route('admin.perfumes.retailers.update', $retailer) }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $retailer->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="owner_name" id="owner_name" value="{{ old('owner_name', $retailer->owner_name) }}"
                ...
                <div>
                    ...
                    <input type="text" name="document" id="document" value="{{ old('document', $retailer->document) }}"
                ...
                <div>
                    ...
                    <input type="text" name="whatsapp" id="whatsapp" value="{{ old('whatsapp', $retailer->whatsapp) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="city" id="city" value="{{ old('city', $retailer->city) }}"
                ...
                <div>
                    ...
                    <select name="state" id="state"
                        ...
                        @foreach(['AC','AL','AP','AM','BA','CE','DF','ES','GO','MA','MT','MS','MG','PA','PB','PR','PE','PI','RJ','RN','RS','RO','RR','SC','SP','SE','TO'] as $uf)
                            <option value="{{ $uf }}" {{ old('state', $retailer->state) === $uf ? 'selected' : '' }}>{{ $uf }}</option>
                        @endforeach
                    </select>
                    @error('state')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $retailer->email) }}"
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="active" {{ old('status', $retailer->status->value) === 'active' ? 'selected' : '' }}>Ativo</option>
                        <option value="inactive" {{ old('status', $retailer->status->value) === 'inactive' ? 'selected' : '' }}>Inativo</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full rounded-lg border border-gray-200 focus:border-pink-500 focus:ring-pink-500 @error('notes') border-red-300 @enderror">{{ old('notes', $retailer->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
