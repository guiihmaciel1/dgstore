<x-perfumes-admin-layout>
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6"
         x-data="{ cost: {{ json_encode((float) (old('cost_price', $product->cost_price) ?? 0)) }}, sale: {{ json_encode((float) (old('sale_price', $product->sale_price) ?? 0)) }} }">
        <form method="POST"
              action="{{ route('admin.perfumes.products.update', $product) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="size_ml" id="size_ml" value="{{ old('size_ml', $product->size_ml) }}" placeholder="ex: 100ml"
                ...
                <div>
                    ...
                    @if($product->photo_url)
                        <div class="mb-2">
                            ...
                            <img src="{{ $product->photo_url }}" alt="{{ $product->name }}" class="h-20 w-20 object-cover rounded-lg border border-gray-200">
                        </div>
                    @endif
                    ...
                    <p class="mt-1 text-xs text-gray-500">Deixe em branco para manter a foto atual</p>
                    @error('photo')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
