<x-app-layout>
    ...
    <div class="py-6">
        <div class="px-6 lg:px-8" x-data="followupPage()">
            ...
            <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.75rem;">
                <div>
                    <h1 style="font-size: 1.5rem; font-weight: 700; color: #e3e3e3;">Agenda de Follow-ups</h1>
                    <div style="display: flex; gap: 0.75rem; margin-top: 0.375rem;">
                        @if($counts['overdue'] > 0)
                            <span style="font-size: 0.75rem; font-weight: 600; color: #fca5a5;">{{ $counts['overdue'] }} atrasado(s)</span>
                        @endif
                        @if($counts['today'] > 0)
                            <span style="font-size: 0.75rem; font-weight: 600; color: #d97706;">{{ $counts['today'] }} para hoje</span>
                        @endif
                        <span style="font-size: 0.75rem; color: #818181;">{{ $counts['pending'] }} pendente(s) total</span>
                    </div>
                </div>
            ...
            <!-- Formulario inline -->
            <div x-show="showForm" x-transition style="background: #141414; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.75rem; padding: 1.25rem; margin-bottom: 1rem;">
                <form method="POST" action="{{ route('followups.store') }}">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 0.75rem; margin-bottom: 0.75rem;">
                        <div>
                            <label style="font-size: 0.7rem; font-weight: 600; color: #818181; display: block; margin-bottom: 2px;">Tipo *</label>
                            <select name="type" required style="width: 100%; padding: 0.4rem 0.5rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; font-size: 0.8rem;">
                        ...
                        <div style="grid-column: span 2 / span 2;">
                            <label style="font-size: 0.7rem; font-weight: 600; color: #818181; display: block; margin-bottom: 2px;">Titulo *</label>
                        ...
                        <div>
                            <label style="font-size: 0.7rem; font-weight: 600; color: #818181; display: block; margin-bottom: 2px;">Data *</label>
                            <input type="date" name="due_date" required value="{{ date('Y-m-d') }}"
                                   style="width: 100%; padding: 0.4rem 0.5rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; font-size: 0.8rem;">
                        </div>
                        <div>
                            <label style="font-size: 0.7rem; font-weight: 600; color: #818181; display: block; margin-bottom: 2px;">Telefone</label>
                            <input type="text" name="phone" placeholder="5517996498338"
                                   style="width: 100%; padding: 0.4rem 0.5rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; font-size: 0.8rem;">
                        </div>
                        <div style="grid-column: 1 / -1;">
                            <label style="font-size: 0.7rem; font-weight: 600; color: #818181; display: block; margin-bottom: 2px;">Descricao</label>
                            <input type="text" name="description" placeholder="Detalhes adicionais (opcional)"
                                   style="width: 100%; padding: 0.4rem 0.5rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; font-size: 0.8rem;">
                        </div>
                    </div>
                    @if($errors->any())
                        <div style="margin-bottom: 0.75rem; color: #fca5a5; font-size: 0.75rem;">
                    ...
                    <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
                        <button @click.prevent="showForm = false" type="button"
                                style="padding: 0.4rem 1rem; font-size: 0.8rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; background: #141414; color: #818181; cursor: pointer;">
                            Cancelar
                        </button>
                        <button type="submit"
            ...
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                        ...
                        <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 0.75rem;">
                            ...
                            <div style="min-width: 0; flex: 1;">
                                <div style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
                                    <span style="font-size: 0.65rem; font-weight: 600; padding: 2px 8px; border-radius: 9999px; background: #222222; color: #a4a4a4;">{{ $f->type->label() }}</span>
                                    @if($isOverdue)
                                    ...
                                    @if($f->status === \App\Domain\Followup\Enums\FollowupStatus::Done)
                                        <span style="font-size: 0.65rem; font-weight: 600; padding: 2px 8px; border-radius: 9999px; background: rgba(16,185,129,0.15); color: #6ee7b7;">CONCLUIDO</span>
                                    @elseif($f->status === \App\Domain\Followup\Enums\FollowupStatus::Cancelled)
                                        <span style="font-size: 0.65rem; font-weight: 600; padding: 2px 8px; border-radius: 9999px; background: #222222; color: #818181;">CANCELADO</span>
                                    @endif
                                </div>
                                <div style="font-size: 0.9375rem; font-weight: 600; color: #e3e3e3; margin-top: 0.25rem;">{{ $f->title }}</div>
                                @if($f->description)
                                    <div style="font-size: 0.8rem; color: #818181; margin-top: 0.125rem;">{{ $f->description }}</div>
                                @endif
                                <div style="font-size: 0.75rem; color: #666666; margin-top: 0.375rem; display: flex; gap: 0.75rem; flex-wrap: wrap;">
                                ...
                                <div style="display: flex; gap: 0.375rem; flex-shrink: 0; flex-wrap: wrap;">
                                    ...
                                    <form method="POST" action="{{ route('followups.cancel', $f) }}" style="display: inline;">
                                        ...
                                        <button type="submit" title="Cancelar"
                                                style="display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 0.375rem; background: #e5e7eb; color: #818181; border: none; cursor: pointer;">
                                    ...
                                    <form method="POST" action="{{ route('followups.destroy', $f) }}" style="display: inline;" onsubmit="return confirm('Remover este follow-up?')">
                                        ...
                                        <button type="submit" title="Excluir"
                                                style="display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 0.375rem; background: rgba(239,68,68,0.1); color: #fca5a5; border: none; cursor: pointer;">
                ...
                @empty
                    <div style="text-align: center; padding: 2.5rem; color: #666666; font-size: 0.875rem;">
                        <svg style="width: 40px; height: 40px; margin: 0 auto 0.75rem; color: #515151;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
