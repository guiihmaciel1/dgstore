    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" x-data="payablesPage()">
            ...
            <div style="background: white; border-radius: 0.75rem; border: 1px solid #e5e7eb; padding: 1rem; margin-bottom: 1.5rem;">
                <form method="GET" action="{{ route('finance.payables') }}" x-data x-ref="filterForm" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 items-end">
                    <div>
                    ...
                    @if(array_filter($filters))
                        <div>
                            <a href="{{ route('finance.payables') }}" style="display: inline-block; padding: 0.375rem 0.75rem; background: white; color: #374151; font-weight: 500; border-radius: 0.375rem; border: 1px solid #d1d5db; text-decoration: none; font-size: 0.8125rem; text-align: center;">Limpar</a>
                        </div>
                    @endif
                </form>
            </div>
