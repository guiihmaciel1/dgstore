...
<div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.75rem;">
    <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center;">
        {{-- Filtro por status --}}
        <div style="display: flex; gap: 0.25rem; background: #222222; border-radius: 0.5rem; padding: 0.125rem;">
            <template x-for="s in [{v:'all',l:'Todas'},{v:'idea',l:'Ideias'},{v:'production',l:'Em Produção'},{v:'published',l:'Publicados'}]" :key="s.v">
                <button type="button" @click="contentFilterStatus = s.v"
                        :style="contentFilterStatus === s.v
                            ? 'padding: 0.375rem 0.75rem; font-size: 0.75rem; font-weight: 600; border: none; border-radius: 0.375rem; cursor: pointer; background: #141414; color: #e3e3e3; box-shadow: none;'
                            : 'padding: 0.375rem 0.75rem; font-size: 0.75rem; font-weight: 500; border: none; border-radius: 0.375rem; cursor: pointer; background: transparent; color: #818181;'"
                        x-text="s.l"></button>
            </template>
        </div>
        ...
        <select x-model="contentFilterType"
                style="padding: 0.375rem 0.5rem; font-size: 0.75rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; outline: none; color: #a4a4a4; background: #141414;">
...
{{-- Estado vazio --}}
<div x-show="filteredContents.length === 0" style="background: #141414; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.75rem; padding: 3rem 2rem; text-align: center;">
    <svg style="width: 3rem; height: 3rem; margin: 0 auto 1rem; color: #515151;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/>
    </svg>
    <p style="color: #818181; font-size: 0.875rem; margin-bottom: 0.5rem;">Nenhuma ideia de conteúdo ainda</p>
    <p style="color: #666666; font-size: 0.8rem;">Clique em "Nova Ideia" ou use o gerador com IA para começar</p>
</div>
...
<div x-show="filteredContents.length > 0"
    ...
    <template x-for="c in filteredContents" :key="c.id">
        <div style="background: #141414; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.75rem; overflow: hidden; display: flex; flex-direction: column; transition: box-shadow 0.15s;"
            ...
            <template x-if="c.image_url">
                <div style="height: 140px; overflow: hidden; background: #1a1a1a;">
            ...
            <div style="padding: 1rem; flex: 1; display: flex; flex-direction: column;">
                ...
                <div style="display: flex; gap: 0.375rem; flex-wrap: wrap; margin-bottom: 0.5rem;">
                    ...
                    <span style="font-size:0.625rem;font-weight:600;padding:2px 8px;border-radius:9999px;background:#222222;color:#818181;"
                ...
                {{-- Título --}}
                <h3 style="font-size: 0.875rem; font-weight: 700; color: #e3e3e3; margin-bottom: 0.375rem; line-height: 1.3;" x-text="c.title"></h3>

                {{-- Descrição truncada --}}
                <p style="font-size: 0.75rem; color: #818181; line-height: 1.5; flex: 1; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;"
                ...
                <template x-if="c.scheduled_at_formatted">
                    <div style="display: flex; align-items: center; gap: 0.25rem; margin-top: 0.5rem;">
                        <svg style="width: 0.75rem; height: 0.75rem; color: #666666;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        <span style="font-size: 0.6875rem; color: #666666;" x-text="c.scheduled_at_formatted"></span>
                    </div>
                </template>
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 0.75rem; padding-top: 0.75rem; border-top: 1px solid #f3f4f6;">
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        <span style="font-size: 0.625rem; color: #515151;" x-text="c.user_name"></span>
                    </div>
                    <div style="display: flex; gap: 0.25rem;">
                        <button type="button" @click="openEditContent(c)"
                                style="padding: 0.25rem; background: none; border: none; cursor: pointer; color: #666666; border-radius: 0.25rem;"
                        ...
                        <button type="button" @click="deleteContent(c.id)"
                                :style="contentDeleting === c.id ? 'padding:0.25rem;background:none;border:none;cursor:default;color:#ef4444;opacity:0.5;' : 'padding:0.25rem;background:none;border:none;cursor:pointer;color:#666666;border-radius:0.25rem;'"
...
<div x-show="contentModal.open" x-cloak
    ...
    <div style="background: #141414; border-radius: 0.75rem; width: 100%; max-width: 540px; position: relative; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.15);">
        <div style="padding: 1.25rem 1.5rem; border-bottom: 1px solid rgba(255,255,255,0.06);">
            <h3 style="font-size: 1rem; font-weight: 700; color: #e3e3e3;" x-text="contentModal.editing ? 'Editar Conteúdo' : 'Nova Ideia de Conteúdo'"></h3>
        </div>
        <div style="padding: 1.5rem; display: flex; flex-direction: column; gap: 1rem;">
            <div>
                <label style="font-size: 0.75rem; font-weight: 600; color: #a4a4a4; display: block; margin-bottom: 0.25rem;">Título *</label>
                <input type="text" x-model="contentForm.title" required
                       style="width: 100%; padding: 0.5rem 0.625rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; font-size: 0.875rem; outline: none; box-sizing: border-box;"
            ...
            <div>
                <label style="font-size: 0.75rem; font-weight: 600; color: #a4a4a4; display: block; margin-bottom: 0.25rem;">Descrição / Roteiro</label>
                <textarea x-model="contentForm.description" rows="4"
                          style="width: 100%; padding: 0.5rem 0.625rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.375rem; font-size: 0.875rem; outline: none; resize: vertical; box-sizing: border-box;"
                          onfocus="this.style.borderColor='#666666'" onblur="this.style.borderColor='rgba(255,255,255,0.06)'"
                          placeholder="Descreva o conteúdo, roteiro, pontos principais..."></textarea>
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
                <div>
                    <label style="font-size: 0.75rem; font-weight: 600; color: #a4a4a4; display: block; margin-bottom: 0.25rem;">Tipo</label>
                ...
                <div>
                    <label style="font-size: 0.75rem; font-weight: 600; color: #a4a4a4; display: block; margin-bottom: 0.25rem;">Plataforma</label>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
                <div>
                    <label style="font-size: 0.75rem; font-weight: 600; color: #a4a4a4; display: block; margin-bottom: 0.25rem;">Status</label>
                ...
                <div>
                    <label style="font-size: 0.75rem; font-weight: 600; color: #a4a4a4; display: block; margin-bottom: 0.25rem;">Data Prevista</label>
            ...
            <template x-if="!contentModal.editing">
                <div>
                    <label style="font-size: 0.75rem; font-weight: 600; color: #a4a4a4; display: block; margin-bottom: 0.25rem;">Imagem de Referência</label>
                    <input type="file" id="content-image-input" accept="image/*"
                           style="width: 100%; font-size: 0.8rem; color: #818181;">
                </div>
            </template>
        </div>
        <div style="padding: 1rem 1.5rem; border-top: 1px solid rgba(255,255,255,0.06); display: flex; justify-content: flex-end; gap: 0.5rem;">
            <button type="button" @click="contentModal.open = false"
                    style="padding: 0.5rem 1rem; background: #222222; color: #a4a4a4; border: none; border-radius: 0.5rem; font-size: 0.8rem; font-weight: 600; cursor: pointer;">
                Cancelar
            </button>
...
<div x-show="aiModal.open" x-cloak
    ...
    <div style="background: #141414; border-radius: 0.75rem; width: 100%; max-width: 680px; position: relative; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.15);">
        <div style="padding: 1.25rem 1.5rem; border-bottom: 1px solid rgba(255,255,255,0.06); display: flex; align-items: center; gap: 0.5rem;">
            ...
            <div>
                <h3 style="font-size: 1rem; font-weight: 700; color: #e3e3e3;">Gerar Ideias com IA</h3>
                <p style="font-size: 0.75rem; color: #666666;">Tendências, curiosidades e novidades Apple</p>
            </div>
        </div>
        <div style="padding: 1.5rem;">
            ...
            <div style="display: flex; gap: 0.5rem; margin-bottom: 1.25rem;">
                <input type="text" x-model="aiTopic"
                       style="flex: 1; padding: 0.625rem 0.75rem; border: 1px solid rgba(255,255,255,0.06); border-radius: 0.5rem; font-size: 0.875rem; outline: none; box-sizing: border-box;"
            ...
            <div x-show="aiLoading" style="text-align: center; padding: 2rem;">
                <div style="display: inline-block; width: 2rem; height: 2rem; border: 3px solid #e5e7eb; border-top-color: #c4b5fd; border-radius: 50%; animation: spin 0.8s linear infinite;"></div>
                <p style="color: #818181; font-size: 0.8rem; margin-top: 0.75rem;">Consultando tendências e gerando ideias...</p>
            </div>
            ...
            <div x-show="aiSuggestions.length > 0 && !aiLoading" style="display: flex; flex-direction: column; gap: 0.75rem;">
                <template x-for="(idea, idx) in aiSuggestions" :key="idx">
                    <div :style="idea.saved
                        ? 'border: 1px solid #d1fae5; border-radius: 0.75rem; padding: 1rem; background: rgba(16,185,129,0.1);'
                        : 'border: 1px solid rgba(255,255,255,0.06); border-radius: 0.75rem; padding: 1rem; background: #1a1a1a; transition: border-color 0.15s;'"
                         onmouseover="if(!this.dataset.saved)this.style.borderColor='#a78bfa'" onmouseout="if(!this.dataset.saved)this.style.borderColor='rgba(255,255,255,0.06)'"
                        ...
                        <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 0.75rem;">
                            <div style="flex: 1;">
                                <div style="display: flex; gap: 0.375rem; margin-bottom: 0.375rem; flex-wrap: wrap;">
                                    ...
                                    <span style="font-size:0.6rem;font-weight:600;padding:2px 6px;border-radius:9999px;background:#222222;color:#818181;"
                                          x-text="idea.platform === 'all' ? 'Todas' : (idea.platform === 'instagram' ? 'Instagram' : (idea.platform === 'tiktok' ? 'TikTok' : 'WhatsApp'))"></span>
                                </div>
                                <h4 style="font-size: 0.8125rem; font-weight: 700; color: #e3e3e3; margin-bottom: 0.25rem;" x-text="idea.title"></h4>
                                <p style="font-size: 0.75rem; color: #818181; line-height: 1.5;" x-text="idea.description"></p>
                            </div>
                ...
                <div style="text-align: center; padding-top: 0.5rem;">
                    <button type="button" @click="generateIdeas()" :disabled="aiLoading"
                            style="padding: 0.5rem 1rem; background: transparent; color: #c4b5fd; border: 1px solid #7c3aed; border-radius: 0.5rem; font-size: 0.8rem; font-weight: 600; cursor: pointer;"
            ...
            <div x-show="aiSuggestions.length === 0 && !aiLoading" style="text-align: center; padding: 1.5rem;">
                <p style="color: #666666; font-size: 0.8rem;">Digite um tema ou clique em "Gerar 5 Ideias" para receber sugestões da IA sobre tendências Apple, curiosidades iPhone, lançamentos e mais.</p>
            </div>
        </div>
        <div style="padding: 0.75rem 1.5rem; border-top: 1px solid rgba(255,255,255,0.06); display: flex; justify-content: flex-end;">
