<nav x-data="{ open: false, commercialOpen: false, financeOpen: false, stockOpen: false, purchasesOpen: false, toolsOpen: false }" class="bg-gray-900 shadow-lg" @click.away="commercialOpen = false; financeOpen = false; stockOpen = false; purchasesOpen = false; toolsOpen = false">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden sm:-my-px sm:ms-6 sm:flex items-center gap-0.5">
                    ...
                    <!-- Link: Perfumes (admin_geral e admin_perfumes) - sistema separado -->
                    @if(auth()->user()->canAccessPerfumesAdmin())
                    <a href="{{ route('admin.perfumes.dashboard') }}"
                       class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg transition text-pink-400 hover:text-white hover:bg-pink-700 border border-pink-500/30 hover:border-transparent">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gray-800">
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(auth()->user()->canAccessPerfumesAdmin())
                <div class="border-t border-gray-700 mt-2 pt-2">
                    <a href="{{ route('admin.perfumes.dashboard') }}" class="flex items-center gap-2 px-4 py-2 text-base font-medium text-pink-400 hover:text-white hover:bg-gray-700">
