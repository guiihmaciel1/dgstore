                    ...
                    <template x-for="snap in simResults" :key="snap.id">
                        <div style="padding: 0.75rem 1rem; border-bottom: 1px solid #f3f4f6; display: flex; align-items: center; justify-content: space-between; gap: 0.75rem;">
                            ...
                            <div style="display: flex; gap: 0.375rem; flex-shrink: 0;">
                                ...
                                <a :href="'{{ route('tools.negotiation-simulator') }}?snap_product=' + encodeURIComponent(snap.product_description) + '&snap_price=' + snap.product_price + (snap.product_cost ? '&snap_cost=' + snap.product_cost : '') + (snap.trade_in_model ? '&snap_tradein_model=' + encodeURIComponent(snap.trade_in_model) + '&snap_tradein_value=' + (snap.trade_in_value || '') + '&snap_tradein_system_value=' + (snap.trade_in_system_value || '') : '')"
                        ...
                        <template x-for="(sale, idx) in sales" :key="sale.id">
                            <div style="padding: 0.875rem 1rem; border-radius: 0.75rem; transition: all 0.2s; margin-bottom: 0.5rem;"
                                ...
                                <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;" x-show="!sale.done">
                                    <template x-if="sale.has_phone">
                                        <a :href="sale.whatsapp_url" target="_blank"
                                           style="display: inline-flex; align-items: center; gap: 0.375rem; padding: 0.4375rem 0.875rem; background: #16a34a; color: white; font-size: 0.75rem; font-weight: 600; border-radius: 0.5rem; text-decoration: none; transition: background 0.15s; line-height: 1;"
