<x-app-layout>
    ...
    <script>
        ...
        function followupModal() {
            const rawSales = @json($followupSales->map(function ($sale) {
                $daysSince = $sale->sold_at->diffInDays(now());
                $productNames = $sale->items->pluck('product_name')->implode(', ') ?: 'Sem produtos';
                $phone = $sale->customer?->phone ?? '';
                $cleanPhone = preg_replace('/\D/', '', $phone);
                if (strlen($cleanPhone) <= 11) $cleanPhone = '55' . $cleanPhone;

                $message = "Olá {$sale->customer?->name}! Aqui é a DG Store.\nFaz {$daysSince} dias que você adquiriu o {$productNames} conosco.\nEstá tudo certo com o aparelho? Precisa de algo?\nAgradecemos pela preferência!";
            ...
            return {
                ...
                async markDone(sale) {
                    sale.loading = true;
                    try {
                        const res = await fetch(`/sales/${sale.id}/followup`, {
                            method: 'POST',
                            headers: {
