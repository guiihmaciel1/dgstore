...
<div class="max-w-5xl mx-auto px-5 sm:px-6">
    ...
    <div class="info-card overflow-hidden mb-8 animate-fade-up">
        <div class="flex flex-col sm:flex-row">
            ...
            {{-- Image --}}
            <div class="relative flex items-center justify-center sm:w-80 md:w-96 p-8 sm:p-12"
                 style="background: linear-gradient(180deg, var(--surface-elevated) 0%, var(--surface) 100%);">
                <div class="absolute inset-0 pointer-events-none"
                     style="background: radial-gradient(ellipse at 50% 50%, rgba(212, 165, 64, 0.04) 0%, transparent 70%);"></div>
                @if($fragrance->image_url)
                    <img src="{{ $fragrance->image_url }}" alt="{{ $fragrance->name }}"
                         class="w-52 sm:w-60 h-auto object-contain relative drop-shadow-2xl">
                @else
                    <div class="w-48 h-64 rounded-2xl flex items-center justify-center" style="background: rgba(255,255,255,0.02);">
                ...
                <div class="absolute bottom-0 left-[10%] right-[10%] h-px"
                     style="background: linear-gradient(90deg, transparent, rgba(212, 165, 64, 0.12), transparent);"></div>
            </div>
            ...
            <div class="flex-1 p-6 sm:p-8 lg:p-10">
                ...
                @if($fragrance->brand)
                    <div class="flex items-center gap-2 mb-3">
                        @if($fragrance->brand_logo_url)
                            <img src="{{ $fragrance->brand_logo_url }}" alt="{{ $fragrance->brand }}" class="h-4 w-auto rounded opacity-50">
                        @endif
                        <span class="text-[10px] uppercase tracking-[0.2em] font-semibold" style="color: var(--gold);">{{ $fragrance->brand }}</span>
                    </div>
                @endif

                {{-- Name --}}
                <h1 class="font-serif text-2xl sm:text-3xl lg:text-4xl font-semibold leading-tight" style="color: var(--cream);">
                    {{ $fragrance->name }}
                </h1>
                ...
                @if($fragrance->sale_price && $fragrance->pix_price)
                    ...
                    @endphp
                    <div class="mt-7 pt-6" style="border-top: 1px solid rgba(212, 165, 64, 0.06);">
                ...
                @if($whatsappNumber)
                    ...
                    <a href="https://wa.me/55{{ $waNumber }}?text={{ $waText }}" target="_blank"
                       class="btn-whatsapp mt-6 inline-flex items-center gap-2.5 px-7 py-3.5 text-white text-sm font-semibold rounded-2xl">
