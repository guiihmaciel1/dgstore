<header x-data="{ mobileMenu: false, userMenu: false }" class="sticky top-0 z-30 bg-white border-b border-gray-200 lg:ml-0">
        ...
        <div class="flex items-center gap-3">
            <div class="relative">
                <button @click="userMenu = !userMenu" @click.away="userMenu = false"
                    ...
                    <div class="w-8 h-8 rounded-full bg-pink-600 flex items-center justify-center text-xs font-bold text-white uppercase">
    ...
    <!-- Mobile navigation -->
    <div x-show="mobileMenu" x-transition x-cloak class="lg:hidden border-t border-gray-200 bg-white">
        <nav class="px-3 py-3 space-y-1">
            ...
            <a href="{{ route('admin.perfumes.samples.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('admin.perfumes.samples.*') ? 'bg-pink-50 text-pink-700' : 'text-gray-600 hover:bg-gray-50' }}">
                Amostras
            </a>
            <a href="{{ route('admin.perfumes.orders.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('admin.perfumes.orders.*') ? 'bg-pink-50 text-pink-700' : 'text-gray-600 hover:bg-gray-50' }}">
                Pedidos
            </a>
